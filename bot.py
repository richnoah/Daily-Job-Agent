...
def run():
    ...
    # Only proceed if there are unseen items
    if new_items:
        for it in new_items[:60]:  # cap to avoid long runs
            if strict_us_remote(it["url"]):
                us_remote_items.append(it)
        print(f"[INFO] US-remote kept: {len(us_remote_items)} / {len(new_items)} checked")
    else:
        print("[INFO] No unseen jobs found in this run.")
